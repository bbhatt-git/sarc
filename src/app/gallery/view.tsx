            ...
            <div className="mt-16 space-y-8 overflow-hidden">
                <Marquee direction="left">
                    {row1Images.map((image, i) => (
                        <div key={`marquee-1-${i}`} className="mx-4 flex-shrink-0 w-80 h-56 glass-card p-2">
                 ...
                 <Marquee direction="right">
                    {row2Images.map((image, i) => (
                         <div key={`marquee-2-${i}`} className="mx-4 flex-shrink-0 w-80 h-56 glass-card p-2">
                            <Image src={image.src} alt={image.hint} data-ai-hint={image.hint} fill sizes="320px" className="object-cover rounded-xl" />
                        </div>
                    ))}
                </Marquee>
                <Marquee direction="left">
                    {row3Images.map((image, i) => (
                        <div key={`marquee-3-${i}`} className="mx-4 flex-shrink-0 w-80 h-56 glass-card p-2">
                            <Image src={image.src} alt={image.hint} data-ai-hint={image.hint} fill sizes="320px" className="object-cover rounded-xl" />
                        </div>
                    ))}
                </Marquee>
                 <Marquee direction="right">
                    {row4Images.map((image, i) => (
                         <div key={`marquee-4-${i}`} className="mx-4 flex-shrink-0 w-80 h-56 glass-card p-2">
                            <Image src={image.src} alt={image.hint} data-ai-hint={image.hint} fill sizes="320px" className="object-cover rounded-xl" />
                        </div>
                    ))}
                </Marquee>
